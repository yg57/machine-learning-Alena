...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        ###########################################################################
        J = np.sum(np.square(np.dot(X,theta)-y))/(2*num_examples) + reg/(2*num_examples)*np.sum(np.square(theta))
    ...
    def grad_loss(self,*args):                                                                          
        ...
        for i in range(dim):
            if i == 0:
                grad = np.dot(np.transpose(X),np.dot(X,theta)-y)/num_examples
            else:
                grad = np.dot(np.transpose(X),np.dot(X,theta)-y)/num_examples +reg/num_examples*theta[i]
...
class LassoLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        J = np.sum(np.square(np.dot(X,theta)-y))/(2*num_examples) + reg/(2*num_examples)*np.sum(np.abs(theta))
    ...
    def grad_loss(self,*args):                                                                          
        ...
        for i in range(dim):
            if i == 0:
                ...
                grad = np.dot(np.transpose(X),np.dot(X,theta)-y)/num_examples +reg/num_examples*np.sign(theta[i])
